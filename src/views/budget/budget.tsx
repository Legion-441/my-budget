...
const BudgetView: React.FC = () => {
  ...
  useEffect(() => {
    const selectedBudget: boolean = budgetsList.some((budget) => budget.id === pickedBudget?.id);
    setIsGrantedAccess(selectedBudget);
  }, [pickedBudget, budgetsList, dispatch]);
  ...
  const deniedAccessBudget: JSX.Element = (
    <Box component="main" sx={{ flexGrow: 1 }}>
      <PaperCard>
        ...
        <p>
          Wygląda na to, że nie masz uprawnień do wyświetlenia budżetu{" "}
          <Chip component={"span"} color="secondary" variant="outlined" size="small" label={id} /> lub budżet nie istnieje.
        </p>
      </PaperCard>
    </Box>
  );
  ...
  const budgetPage: JSX.Element = (
    <Box sx={{ display: "flex", minHeight: "100vh", bgcolor: "background.default" }}>
      {pickedBudget && <ExtendableNavBar selectedSubPage={selectedSubPage} />}
      <Box component="main" sx={{ flexGrow: 1 }}>
        <PageContainer>
          <Outlet />
        </PageContainer>
      </Box>
      {pickedBudget && <MobileBottomNavigation selectedSubPage={selectedSubPage} />}
    </Box>
  );
  ...
  if (pickedBudget && isGrantedAccess) {
    return budgetPage;
  } else {
